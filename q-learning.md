...
Q-learning作为model-free算法，无法直接使用求和形式 `Q*(s,a) = r(s,a) + γ ∑_{s'∈S} p(s'|s,a) max_{a'} Q*(s',a')`，因为它不知道环境模型——既不知道状态转移概率 p(s'|s,a)，也不知道奖励函数 r(s,a)，因此无法计算这个求和。但Q-learning的目标仍然是找到满足期望形式 `Q*(s,a) = 𝔼[R_{t+1} + γ max_a Q*(S_{t+1},a)]` 的最优Q函数，它通过与环境交互获得采样 `(s,a,R_{t+1},s_{t+1})`，用单个样本 `R_{t+1} + γ max_a Q(s_{t+1},a)` 作为期望的无偏估计进行增量更新，通过大量采样和迭代，利用大数定律使Q值逐渐收敛到满足贝尔曼最优方程的Q*。
...
- **作用**：在Q-learning更新公式中，用于计算 max_a Q(s_{t+1}, a)
  - 即在下一状态s_{t+1}时，选择使Q值最大的动作
  - 这个最大值用于构建TD target：R_{t+1} + γ max_a Q(s_{t+1}, a)
  - 体现了"假设未来按最优策略行动"的思想
...
采样(s_t, a_t, R_{t+1}, s_{t+1})后，采用增量更新的方式：Q_new = Q_old + α(target - Q_old)，其中α为学习率（标准Q-learning中0 < α ≤ 1）

### 学习率α的影响分析
1. **α=1**：完全替换，直接跳到目标：Q_new = 𝒯Q_old
2. **α=0**：完全不动：Q_new = Q_old
3. **0<α<1**：部分朝𝒯Q_old移动，保守更新
...
### 收敛性的前提条件
Q-learning收敛到最优Q*需要满足以下条件（Watkins & Dayan, 1992）：
1. 有限的状态和动作空间
2. 所有状态-动作对被无限次访问
3. 学习率满足Robbins-Monro条件：∑α_t = ∞ 且 ∑α_t² < ∞
4. 折扣因子 0 ≤ γ < 1
...
在更新公式 Q_new(s_t, a_t) = Q_old(s_t, a_t) + α[R_{t+1} + γ max_a Q(s_{t+1}, a) - Q_old(s_t, a_t)] 中：
- Q_old(s_t, a_t) 是确定的（当前Q表中的值）
- 但 s_{t+1} 是从状态转移概率分布 P(·|s_t, a_t) 中随机采样得到的
- 不同的 s_{t+1} 导致不同的 max_a Q(s_{t+1}, a) 值
