...
- **作用**：在Q-learning更新公式中，用于计算 max_{a'} Q(s_{t+1}, a')
  - 即在下一状态s_{t+1}时，选择使Q值最大的动作
  - 这个最大值用于构建TD target：R_{t+1} + γ max_{a'} Q(s_{t+1}, a')
  - 体现了"假设未来按最优策略行动"的思想
...
```
- **作用**：将任意Q函数映射为新的Q函数，表示理想的值函数更新
- **意义**：如果能计算完整期望，Q值应该朝这个方向更新
...
```
- **作用**：定义了最优动作价值函数Q*的特征
- **意义**：Q*是贝尔曼算子𝒯的不动点，即应用算子后保持不变

**3. 压缩映射性质**
```
||𝒯Q₁ - 𝒯Q₂||_∞ ≤ γ||Q₁ - Q₂||_∞
```
- **作用**：保证算子𝒯是压缩的，每次应用会缩小Q函数间的距离
- **意义**：这是收敛性的关键，确保迭代过程会收敛

**4. 无穷范数||·||_∞**
```
||Q||_∞ = max_{s,a} |Q(s,a)|
```
- **定义**：所有状态-动作对的Q值绝对值的最大值
- **作用**：度量Q函数间的最大偏差（最坏情况误差）
- **意义**：确保所有(s,a)对都收敛，不仅是平均意义

**5. Banach不动点定理**
- **内容**：完备度量空间中的压缩映射有唯一不动点
- **作用**：保证Q*的存在性和唯一性
- **意义**：从理论上确保Q-learning有唯一的收敛目标
